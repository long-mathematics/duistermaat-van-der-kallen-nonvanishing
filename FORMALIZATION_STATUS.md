...
| `std:sa`: semialgebraic standard input | `PolynomialTail`; `SemialgebraicSets`; `SemialgebraicLine`; `SemialgebraicLineFamilies`; `SemialgebraicCurveRegularity`; `ComplexSemialgebraic`; `SemialgebraicObligations` | PARTIAL | Detailed component inventory below. No Hardt/projection input is assumed as a project axiom. |
...
| `cor:bounded-volume`: uniform bounded-family volume | `ScalarVariation`; `CurveVariation`; `ContinuousCurveRectifiability` | PARTIAL | For given bounded continuous parametrized curve families with semialgebraic coordinate graphs, uniform integrated-speed and metric-variation bounds are proved from line-fiber counts and derived finite-exception regularity. No C¹, projection, integrability, or uniform exceptional-count premise is imposed. This does not yet prove the general Hausdorff-volume statement for arbitrary semialgebraic set families or the higher-dimensional complex-form bound. |
| `lem:connecting-paths`: uniform compact-family connecting paths | `FiniteCoverPaths`; `PolygonalChain`; `SimplicialPaths`; `SpherePathObligation`; `SphereC1ChainObligation` | PARTIAL | A fixed finite geometric simplicial complex is compact, has at most one connected component per face, and admits actual C¹ connecting chains with uniform piece count and length. The polygonal paths remain in the model and each segment lies in a face. Given bounded continuous curve families with jointly semialgebraic coordinate graphs have uniform metric-variation bounds in `uniform_continuous_semialgebraic_rectifiable_curves`, with regularity derived. No uniform exceptional-point count is needed for this length estimate. Constructing the model families by Hardt and semialgebraic triangulation, obtaining their joint graph descriptions, matching the curves to actual sphere components, and converting to endpoint-C¹ chains if using the existing `SphereC1ChainObligation` interface remain open. Both sphere-path obligations are still unproved; no equivalence between their rectifiable and finite-C¹ formulations is claimed. |
...
| Algebraic relation for a semialgebraic scalar graph | PROVED: `IsSemialgebraic.frontier_polynomial` contains the entire frontier in the zero set of a nonzero polynomial, by finite Boolean induction. Empty-interior sets inherit a nonzero vanishing polynomial. `scalar_graph_empty_interior` and `semialgebraic_scalar_graph_polynomial` apply this to arbitrary scalar graphs, without continuity or projection. `BivariatePolynomials` gives exact evaluation-preserving conversion and degree stability outside finitely many input values. |
| Regularity of an individual continuous semialgebraic curve | PROVED special case: `continuous_semialgebraic_scalar_smooth_outside_finite` gives actual `ContDiffAt ℝ ⊤` at every interior-domain point outside a finite set. `continuous_semialgebraic_curve_smooth_outside_finite` gives one finite exceptional set for a Euclidean curve with semialgebraic coordinate graphs. Continuity is required only on the given domain. The proof derives a polynomial relation, removes repeated factors over the fraction field, clears denominators, excludes resultant/leading-coefficient zeros, and identifies the continuous selection with the implicit smooth branch. No projection, Hardt, or assumed C¹ regularity is used. This gives no uniform exceptional-set cardinality in a parameter family and no C¹ extension or reparametrization at exceptional points or endpoints. The uniform length result below now avoids needing a uniform exceptional-set cardinality. |
...
| Countable-piece Euclidean Jacobian bound | PROVED analytic implication: `tsum_measure_le_mul_measure_union_of_ae_multiplicity` controls countable overlaps, and `image_membership_count_le_fiber` derives the overlap count from disjoint source pieces and actual fiber cardinality. `lintegral_abs_det_le_mul_image_of_ae_finite_fibers` uses mathlib change of variables on each injective measurable piece. The target measure may be infinite and a null exceptional target set is permitted. Existence of suitable pieces and almost-everywhere fiber finiteness are explicit premises. The earlier family theorem is CONDITIONAL on these premises and coordinate projection; the new C¹ theorem below derives the pieces and finite fibers. |
| Countable semialgebraic fibers | PROVED line case: `finite_polynomial_preimage_of_countable` and `finite_line_fiber_of_countable` use the finite convex cover and total disconnectedness of a countable real set. Arbitrary finite-dimensional fibers are CONDITIONAL on projection in `finite_fiber_of_countable_of_projection`, by injecting into the finite product of coordinate images. |
...
| FTC and metric variation across finite exceptional sets | PROVED: `integral_eq_sub_of_hasDerivAt_off_finite` splits at the finitely many exceptional points, retaining continuity and integrability but requiring no derivatives there. `eVariationOn_le_integral_speed_off_finite` applies this on every partition interval. The earlier no-exception estimate is a specialization. |
| Uniform variation with finite exceptional source sets | PROVED: `uniform_semialgebraic_scalar_variation_off_finite` and the three corresponding curve estimates retain the semialgebraic graph on the full source. Almost-everywhere countable regular fibers plus finitely many omitted points make the full fibers countable, hence finite by the semialgebraic line theorem. Their uniform full-graph cardinal bound also bounds the restricted fibers. Exceptional sets need neither jointly semialgebraic descriptions nor uniform cardinality bounds. Earlier C¹ interfaces are empty-exception specializations with unchanged statements. |
| Metric rectifiability of supplied continuous curve families | PROVED: `continuous_semialgebraic_curve_family_finite_exceptions` derives finite exceptional sets by polynomial specialization of the joint graphs. `uniform_continuous_semialgebraic_finite_length` proves integrability of the actual derivative over the full closed interval and one real integrated-speed bound for all parameters. `uniform_continuous_semialgebraic_rectifiable_curves` proves actual bounded variation and a single metric-length bound from continuity, joint coordinate-graph semialgebraicity, and bounded image. No C¹, exceptional-set, integrability, projection, or uniform Lipschitz premise is assumed. Finite-set removal preserves the actual integrals; FTC across the exceptional points bounds the full original curve. This constructs neither the connecting curves nor endpoint-C¹ reparametrizations. |
...
36. `LocalInjectivePartition` constructs a countable open subcover of local
    injectivity neighborhoods, intersects with the measurable source, and
    disjointifies. Fibers of a locally injective map are discrete and therefore
    countable in a second-countable space. At nonsingular C¹ points local
    injectivity follows from the inverse function theorem.
    `RegularJacobian` applies mathlib's equal-dimensional critical-image
    measure-zero result to obtain almost-everywhere countable whole fibers.
    Derivative continuity on the source follows from the given local C¹
    regularity; no globally continuous derivative is imposed. The singular
    source contributes zero Jacobian, and the countable partition of the
    regular source satisfies the actual change-of-variables hypotheses.
    `CountableSemialgebraic` proves countable fibers finite by the line interval
    decomposition and coordinate projections. `SemialgebraicJacobian` combines
    these facts with the uniform multiplicity theorem, obtaining the flat
    Euclidean C¹ family estimate with projection as its only unproved geometric
    input. Its uniform constant precedes every original family parameter.
    This advances item 35 by deriving its partition and exceptional-target
...
39. `SemialgebraicBoundary`, `BivariatePolynomials`, `SeparableRelation`,
    `AlgebraicCurveRegularity`, and `SemialgebraicCurveRegularity` prove the
    individual-curve regularity component without assuming stratification.
    A Boolean sign formula has algebraic frontier; the graph's empty interior
    gives a nonzero relation. Over the input rational-function field, choose
    a squarefree divisor whose power is divisible by the original polynomial.
    Clear denominators to obtain a polynomial relation with simple roots
    outside finitely many zeros of an explicit nonzero input polynomial.
    The resultant excludes common roots with the output derivative. The
    ordinary real implicit-function theorem, together with continuity of the
    original selection, proves smoothness of that actual selection. Finite
    unions of exceptional sets handle all Euclidean coordinates.
    This is a proved special case of semialgebraic regularity supporting the
    manuscript path construction, not a replacement for Hardt, triangulation,
    or common-radius finiteness. It does not prove uniform exceptional counts
    over parameters, bounded endpoint derivatives, or actual endpoint-smooth
    C¹ chains. The nonzero relation and finite exceptional set are conclusions,
    not premises of the final semialgebraic theorem. The fraction-field algebra
    is used only for local curve regularity, with no Puiseux or infinity route.
40. `FiniteExceptionFTC` extends FTC and the metric-variation estimate across
    finitely many exceptional source points by interval splitting and integral
    additivity. `ScalarVariation` and `CurveVariation` now bound integrals on
    the complements of arbitrary finite exceptional sets, while keeping the
    semialgebraic graph on the full source. The full fibers are almost surely
    countable after adding back finitely many omitted points, and therefore
    finite with a uniform bound supplied by the original graph. This avoids
    imposing a jointly semialgebraic exceptional set or a uniform bound on its
    cardinality. Original C¹ theorem statements are preserved as specializations.
    `ContinuousCurveRectifiability` specializes each joint graph polynomially,
    derives finite-exception smoothness, proves full-interval integrability of
    the actual derivative, and bounds the original continuous curve's metric
    variation. This discharges the previous assumed interior-C¹ regularity in
    the parametrized-family length theorem. Uniform exceptional-point counts
    remain unproved but are not required by this argument. No uniform Lipschitz
    control of a Hardt map is introduced; Hardt/triangulation and actual path
    construction remain open. This is a documented refinement of the
    manuscript's one-dimensional projection/volume step, not full coverage of
    its Hausdorff-volume or general-stratum statements.
41. No mathematical manuscript corrections or changes were made. No alternate
...
The checkpoint has 115 mathematical module files plus the root umbrella and
